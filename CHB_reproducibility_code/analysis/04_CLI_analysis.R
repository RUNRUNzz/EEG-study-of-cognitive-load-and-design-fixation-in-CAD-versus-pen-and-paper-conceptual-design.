...
# Paths -------------------------------------------------------------------
data_file <- file.path("data", "eeg_features.csv")
results_dir <- "results"
figures_dir <- "figures"
...
panel_a <- ggplot(plot_long, aes(Condition, Delta_CLI, group = Participant)) +
  ...
  geom_point(
    data = condition_summary,
    aes(Condition, Mean),
    inherit.aes = FALSE, shape = 21, size = 4.7, fill = "white"
  ) +
...
panel_b <- ggplot(cli_paired, aes(x = 1, y = Difference_CAD_minus_PP)) +
  ...
  geom_point(
    data = diff_summary,
    aes(x = 1, y = Mean),
    inherit.aes = FALSE, shape = 23, size = 5, fill = "white"
  ) +
  scale_x_continuous(breaks = 1, labels = "CAD - PP", limits = c(0.75, 1.25)) +
  labs(x = NULL, y = expression(Delta * "CLI difference")) +
  theme_classic(base_size = 14)

cli_plot <- panel_a + panel_b + plot_layout(widths = c(1.35, 1)) +
  plot_annotation(tag_levels = "A")

print(cli_plot)

ggsave(file.path(figures_dir, "CLI_CAD_PP.png"), cli_plot,
       width = 10, height = 5.2, dpi = 600, bg = "white")
ggsave(file.path(figures_dir, "CLI_CAD_PP.pdf"), cli_plot,
       width = 10, height = 5.2)
